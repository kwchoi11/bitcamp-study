...
public class ServerApp {
  ...
  public ServerApp(int port) throws Exception {
    ...
    // Dao 객체 생성 및 보관
    daoMap.put("member", new MemberListDao("memberjson"));
  ...
  public void execute() throws Exception {
    ...
    while (true) {
      ...
      if (dao == null) {
        // 만약 데이터를 처리할 DAO를 찾지 못 한다면 오류 정보를 클라이언트에게 보낸다.
        out.writeUTF(
      ...
      if (method == null) {
        // 만약 클라이언트가 요청한 메서드를 찾지 못 한다면, 오류 정보를 클라이언트에게 보낸다.
        out.writeUTF(
